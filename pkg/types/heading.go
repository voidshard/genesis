...
func CounterClockwiseHeadings(in ...Heading) []Heading {
	res := make([]Heading, len(in))
	for i, h := range in {
		res[i] = h.Right()
	}
	return res
...
func ClockwiseHeadings(in ...Heading) []Heading {
	res := make([]Heading, len(in))
	for i, h := range in {
		res[i] = h.Left()
	}
...
func ReverseHeadings(in ...Heading) []Heading {
	res := make([]Heading, len(in))
	for i, h := range in {
		res[i] = h.Opposite()
	}
