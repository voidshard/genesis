// Taken from https://github.com/esimov/stackblur-go
// Didn't seem adding a dependency for one file :shrug:
...
// smoothImage takes the source image and returns it's blurred version by applying the blur radius defined as parameter.
func smoothImage(src image.Image, radius uint32) (*image.NRGBA, error) {
	var (
		stackEnd *blurStack
		stackIn  *blurStack
		stackOut *blurStack
