...
// NewPerlin generates a single perlin noise map of size (fx,fy).
...
// it's absolute value.
func NewPerlin(fx, fy int, scale float64, stretch bool) *image.Gray {
	...
	im := image.NewGray(image.Rect(0, 0, x, y))
	for dx := 0; dx < x; dx++ {
		for dy := 0; dy < y; dy++ {
			n := (noise[(dy*x)+dx] - min) * (1 / (max - min))
			im.Set(dx, dy, color.Gray{uint8(n * math.MaxUint8)})
		}
	}
	if fx != x || fy != y {
		im = (resizeImage(float64(fx), float64(fy), im)).(*image.Gray)
	}
...
func lerpPerlin(a, b, v float32) float32 {
	return a*(1-v) + b*v
}
...
func (n2d *noise2DContext) Get(x, y float32) float32 {
	...
	vx0 := lerpPerlin(v0, v1, fx)
	vx1 := lerpPerlin(v2, v3, fx)
	fy := smooth(y - n2d.origins[0].Y)
	return lerpPerlin(vx0, vx1, fy)
}
